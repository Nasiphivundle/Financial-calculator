...
inv_interest_simple = tk.Radiobutton(root, text="Simple interest", font=("Times New Roman", 13),
                                     variable=inv_interest_var, value="Simple")
inv_interest_simple.grid(row=4, column=1, padx=10, pady=5, sticky="W")
inv_interest_compound = tk.Radiobutton(root, text="Compound interest", font=("Times New Roman", 12),
                                       variable=inv_interest_var, value="Compound")
...
def calculate_bond():
    ...
    annual_interest_rate = float(bond_rate_entry.get())
    ...
    repayment = (monthly_interest_rate * bond_value) / (1 - math.pow(1 + monthly_interest_rate, -n))

    bond_answer_label.config(text=f"Your monthly payment will be R{repayment:.2f}.")
